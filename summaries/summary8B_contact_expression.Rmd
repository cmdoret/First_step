...
for(t in levels(as.factor(comp_lines$cell.line))){
    ...
    gl<-append(gl,c("comp1","comp2","comp3"))
}
wilcox_p <- data.frame(pval=pl,cell.line=tl,gentype=gl)
...
ggplot(data=comp_lines[comp_lines$diam!=0 & comp_lines$gentype!="eplincRNA",],aes(x=gentype,y=diam,fill=gentype))+
  ...
  geom_boxplot()+
  geom_text(data = wilcox_p, x=c(1.5,2,2.5),y=c(100,100,100),label=wilcox_p$pval)+
