...
store_corr <- store_corr[order(store_corr$p.value),]
kable(store_corr)
```

#Comparisons sacross cell lines.

Comparing gene-TAD contact across all cell lines.
...
ggplot(data=comp_lines[comp_lines$diam!=0 & comp_lines$gentype!="eplincRNA",],aes(x=gentype,y=diam,fill=gentype))+
  ...
  scale_fill_manual(values=c("darkblue","darkred","darkred"))+
  geom_boxplot()+
  theme_bw()
...
ggplot(data=comp_lines[comp_lines$diam!=0 & comp_lines$gentype!="epPCG",],aes(x=gentype,y=diam))+
  facet_grid(~cell.line)+
  scale_fill_manual(values=c("darkblue","darkred","darkred"))+
  geom_boxplot()+
  theme_bw()
